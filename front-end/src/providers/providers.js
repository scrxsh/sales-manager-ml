        ...
        .then((result) => {
            listaProveedores = result
                result.forEach(element => {
                    tBody.innerHTML +=
                    `
                    <tr class="odd:bg-white even:bg-gray-50 border-b dark:bg-gray-900">
                        <th scope="row" class="px-4 py-2 text-gray-900 dark:text-gray-50 whitespace-nowrap">E0${element.id}</th>
                        <th scope="row" class="px-4 py-2 dark:text-gray-200">${element.nombreEmpresa}</th>
                        <td class="px-4 py-2 dark:text-gray-300">${element.encargado}</td>
                        <td class="px-4 py-2 dark:text-gray-300">${element.telefono}</td>
                        <td class="px-4 py-2 dark:text-gray-300">${element.correo}</td>
                        <td class="px-4 py-2 dark:text-gray-300">${element.direccion}</td>
                        <td class="px-4 py-2 -ml-5 flex space-x-2">
                            <i class="fa-solid fa-pen-circle text-green-600 dark:text-green-400 text-3xl cursor-pointer" onclick="editarProveedor('modalProvider', ${element.id})"></i>
